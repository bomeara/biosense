...
DoUnivariateOUwieDredge <- function(data, phy) {
	data.means.with.error <- GetSpeciesMeansWithError(data)	
}

#' Computes the mean and standard error for values for each species for a univariate trait.
#' 
#' @param data A data.frame with a species column (with repeats) and measurements
